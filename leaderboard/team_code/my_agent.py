...
from timm.models import create_model,model
from team_code.utils import lidar_to_histogram_features, transform_2d_points
from team_code.planner import RoutePlanner
from team_code.my_controller import Controller
...
class DisplayInterface(object):
    ...
    def run_interface(self, input_data):
        ...
        surface[400:600,800:1200] = input_data['saliency_show']
        ...
        surface = cv2.putText(surface, 'Driver Attention Prediction', (850,420), cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,0,0), 2)
...
class SDFusionAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        self.salidency_model = model.get_model()().cuda()
        ...
        pretrained_dict = torch.load(path_to_model_file)["state_dict"]
        ...
        if SAVE_PATH is not None:
            ...
            (self.save_path / "left_rgb").mkdir(parents=True, exist_ok=False)
            (self.save_path / "right_rgb").mkdir(parents=True, exist_ok=False)
            (self.save_path / "saliency").mkdir(parents=True, exist_ok=False)
            (self.save_path / "map").mkdir(parents=True, exist_ok=False)
    ...
    def run_step(self, input_data, timestamp):
        ...
        model_kwargs = {}
        this_frame_seq = input_data["rgb"].unsqueeze(0)

        model_kwargs['target_size'] = (tick_data["rgb"].shape[0], tick_data["rgb"].shape[1])
        ...
        smap = (smap / smap.max())
        smap1 = copy.deepcopy(smap)
        smap1 = smap1.expand(3, -1, -1).contiguous().permute(1, 2, 0)
        smap1 = smap1.detach().cpu().numpy()
        smap1 = (smap1 * 255).astype(np.uint8)

        smap_color = cv2.applyColorMap(smap1, cv2.COLORMAP_JET)
        img_smap = Image.fromarray(cv2.cvtColor(smap_color, cv2.COLOR_BGR2RGB))
        raw_data = Image.fromarray(tick_data["rgb"])
        images = Image.blend(raw_data, img_smap, 0.4)


        masked_map = adjust_salience_dynamic_range(smap).cpu()
        alpha = self.config.alpha_DA / 255
        masked_map = to_tensor(tick_data["rgb"]) * (1 - alpha + (alpha * masked_map))
        masked_map = torch.clamp(masked_map, 0, 1)
        masked_map = to_pil_image(masked_map)
        if self.rgb_center_transform is not None:
            rgb_main = self.rgb_center_transform(masked_map).unsqueeze(0).cuda().float()
        input_data["rgb_center"] = rgb_main

        input_data['raw_data'] = torch.from_numpy(np.array(tick_data["rgb"])).unsqueeze(0).cuda()
        ...
        tick_data["rgb_left_raw"] = copy.deepcopy(tick_data["rgb_left"])
        tick_data["rgb_right_raw"] = copy.deepcopy(tick_data["rgb_right"]) 
        tick_data['saliency'] = copy.deepcopy(np.array(images))
        tick_data['saliency_show'] = cv2.resize((np.array(images)), (400, 200))
    ...
    def save(self, tick_data):
        ...
        Image.fromarray(tick_data["rgb_raw"]).save(
            self.save_path / "rgb" / ("%04d.jpg" % frame)
        )
        Image.fromarray(tick_data["rgb_left_raw"]).save(
            self.save_path / "left_rgb" / ("%04d.jpg" % frame)
        )
        Image.fromarray(tick_data["rgb_right_raw"]).save(
            self.save_path / "right_rgb" / ("%04d.jpg" % frame)
        )
        Image.fromarray(tick_data['saliency']).save(
            self.save_path / "saliency" / ("%04d.jpg" % frame)
        )
        Image.fromarray(tick_data['map']).save(
            self.save_path / "map" / ("%04d.jpg" % frame)
        )
        return
