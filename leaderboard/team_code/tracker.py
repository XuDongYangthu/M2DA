...
from team_code.nms import find_peak_box_with_nms
...
class Tracker():
    ...
    def update_and_predict(self, det_data, pos, theta, step, merge_precent=0.4):
        ...
        box_ids, box_info = find_peak_box_with_nms(det_data)
        objects_info = []
        R = np.array(
    ...
    def _predict(self, updates_ids):
        ...
        for each_id in updates_ids:
            ...
            for i in range(len(to.historical_steps)-3, -1, -1):
                ...
                speed = 0.5 * self.frequency * np.sqrt((prev_pos[0]-cur_pos[0])**2+(prev_pos[1]-cur_pos[1])**2) / (to.historical_steps[i+2]-to.historical_steps[i])
                speed = np.clip(speed-0.2, 0, 7)
                avg_speed.extend([speed]*(to.historical_steps[i+2]-to.historical_steps[i]))
                if speed > 1:
