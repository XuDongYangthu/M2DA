...
class BaseAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        self.weather_id = self.config.get("weather", 5)
        ...
        self.rgb_only = self.config.get("rgb_only", False)
        ...
        if SAVE_PATH is not None:
            ...
            for pos in ["rear", "right_rear", "left_rear"]:
    ...
    def tick(self, input_data):
        ...
        rgb_left_rear = cv2.cvtColor(input_data["rgb_left_rear"][1][:, :, :3], cv2.COLOR_BGR2RGB)
        rgb_right_rear = cv2.cvtColor(input_data["rgb_right_rear"][1][:, :, :3], cv2.COLOR_BGR2RGB)
        ...
        if self.rgb_only:
            ...
            return {
                ...
                "rgb_rear": rgb_rear,
                "rgb_right_rear": rgb_right_rear,
                "rgb_left_rear": rgb_left_rear,
                "rgb_left": rgb_left,
                "seg_left": seg["left"],
    ...
    def save(
        ...
        for pos in ["front", "left", "right", "rear", "right_rear", "left_rear"]:
            name = "rgb_" + pos
            Image.fromarray(tick_data[name]).save(
                self.save_path / name / ("%04d.jpg" % frame)
            )
            if not self.rgb_only and pos != "rear" and pos != "right_rear" and pos != "left_rear":
        ...
        if not self.rgb_only:
            ...
            np.save(
                self.save_path / "radar" / ("%04d.npy" % frame),
                tick_data["radar"],
                allow_pickle=True,
            )
