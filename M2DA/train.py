...
from timm.data import create_carla_dataset, create_carla_loader, create_carla_dataset
...
class WaypointL1Loss:
    ...
    def __call__(self, output, target):
        ...
        output = output.clone()  # 或者 output = output.detach()
        target = target.clone()  # 或者 target = target.detach()
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if args.distributed:
                ...
                if isinstance(loss_velocity, torch.Tensor):
                    reduced_loss_velocity = reduce_tensor(loss_velocity.data, args.world_size)
                else:
                    continue
                    loss_velocity = torch.tensor(loss_velocity)
                    reduced_loss_velocity = reduce_tensor(loss_velocity.data, args.world_size)
