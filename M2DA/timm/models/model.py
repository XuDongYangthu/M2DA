...
class BaseModel(nn.Module):
    ...
    def load_best_trained_weights(self,):
        ...
        path_weight = os.path.dirname(os.path.dirname(__file__))
        self.load_state_dict(torch.load(path_weight + '/weights/weights_best.pth', map_location='cpu'))
...
class DANet_attention(nn.Module):
    ...
    def __init__(self, channels_4x, sources):
        super(DANet_attention, self).__init__()
        ### refer from the DANet
        # channels_4x=64
        input_channels = channels_4x
        inter_channels = channels_4x //4
        inter_channels2  = channels_4x//16
        out_channels = 1
        # self.conv5a = nn.Sequential(nn.Conv2d(input_channels, inter_channels, 3, padding=1, bias=False),
        #                         self.get_bn_module(inter_channels, sources),
        #                         nn.BatchNorm2d(inter_channels),
        #                         nn.ReLU())
        
        self.conv5c = nn.Sequential(nn.Conv2d(input_channels, inter_channels, 3, padding=1, bias=False),
                                self.get_bn_module(inter_channels, sources),
                                # nn.BatchNorm2d(inter_channels),
                                nn.ReLU())
        ...
        # self.conv51 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels2, 3, padding=1, bias=False),
        #                         nn.BatchNorm2d(inter_channels2),
        #                         nn.ReLU())
        # self.conv52 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels2, 3, padding=1, bias=False),
        #                         nn.BatchNorm2d(inter_channels2),
        #                         nn.ReLU())
    ...
    def forward(self, x):
        ...
        im_feat_1x = self.conv8(sc_feat)

        return im_feat_1x
    def get_bn_module(self, num_features, source, **kwargs):
        return DomainBatchNorm2d(
            num_features, source, momenta=[0.01]*4, **kwargs)

### attention module
class DANet_attention2(nn.Module):

    def __init__(self, channels_4x, sources):
        super(DANet_attention2, self).__init__()
        ### refer from the DANet
        # channels_4x=64
        input_channels = channels_4x
        inter_channels = channels_4x
        inter_channels2  = channels_4x//16
        out_channels = 1
        # self.conv5a = nn.Sequential(nn.Conv2d(input_channels, inter_channels, 3, padding=1, bias=False),
        #                         self.get_bn_module(inter_channels, sources),
    ...
    def forward(self, x):
        ...
        # feat_sum = sa_conv+sc_conv
        # feat_sum = sa_feat+sc_feat
        # im_feat_1x = self.conv8(feat_sum)
...
class Attention_upsampling2(nn.Module):

    def __init__(self, channels_4x):
        super(Attention_upsampling2, self).__init__()
        ...
        self.sc = CAM_Module(input_channels )
        # self.conv52 = nn.Sequential(nn.Conv2d(inter_channels, input_channels, 3, padding=1, bias=False),
...
class Attention_upsampling1(nn.Module):

    def __init__(self, channels_4x):
        super(Attention_upsampling1, self).__init__()

        ### refer from the DANet
        input_channels = channels_4x

        self.sa = PAM_Module(input_channels)
...
class UNISAL(BaseModel, utils.KwConfigClass):
    ...
    def __init__(self,
                 rnn_input_channels=256, rnn_hidden_channels=128,
                 cnn_cfg=None,
                 rnn_cfg=None,
                 res_rnn=True,
                 bypass_rnn=True,
        ...
        for source_str in self.sources:
            ...
            #     nn.Sequential(*[
            #         nn.Conv2d(channels_4x//2, 1, 1, bias=True)
            #     ]))
            ...
            ### refer from the DANet
            self.__setattr__(
                'atten_postcnn'+source_str, DANet_attention2(self.cnn.out_channels, self.sources)
            )

            # self.__setattr__(
            #     'atten_upsam2'+source_str, Attention_upsampling2(channels_2x + channels_4x)
            # )

            self.__setattr__(
                'atten_upsam1'+source_str, Attention_upsampling1(rnn_input_channels + channels_2x)
            )

            self.__setattr__(
                'attention'+source_str, DANet_attention(channels_4x, self.sources)
            )
    ...
    def forward(self, x, target_size=None, h0=None, return_hidden=False,
        ...
        for t, img in enumerate(torch.unbind(x, dim=1)):
            # resnet50 设置replace_stride_with_dilation=[False, True, False]
            # [2, 2048, 14, 24]
            # [2, 1024, 28, 48]
            # [2, 512, 28, 48]
            ...
            im_feat_1x = self.__getattr__(
                'atten_postcnn' + source_str)(
                im_feat_1x)
