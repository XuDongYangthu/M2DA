...
from ..data.carla_loader import salidency_trans
import os
...
def conv_1(x):
    return nn.Conv2d(x.shape[1],x.shape[1],1,1).to(device)(x)
...
class FusionLayer(nn.Module):
    ...
    def forward(
        ...
        tgt2 = self.multihead_attn(
            query=self.with_pos_embed(bev_tgt, query_pos),
            key=self.with_pos_embed(lidar, pos),
            value=lidar,
            attn_mask=memory_mask,
            key_padding_mask=memory_key_padding_mask,
            )[0]
        bev_tgt = bev_tgt + self.dropout1(tgt2)
        bev_tgt = self.norm1(bev_tgt)
        
        tgt2 = self.multihead_attn(
            query=self.with_pos_embed(bev_tgt, query_pos),
            key=self.with_pos_embed(image, pos),
            value=image,
            attn_mask=memory_mask,
            key_padding_mask=memory_key_padding_mask,
            )[0]
        ...
        q = k = self.with_pos_embed(bev_tgt, query_pos)
        tgt2 = self.self_attn(
            q, k, value=bev_tgt, attn_mask=tgt_mask, key_padding_mask=tgt_key_padding_mask
        )[0]
        bev_tgt = bev_tgt + self.dropout1(tgt2)
        bev_tgt = self.norm1(bev_tgt)

        tgt2 = self.linear2(self.dropout(self.activation(self.linear1(bev_tgt))))
        bev_tgt = bev_tgt + self.dropout3(tgt2)
        bev_tgt = self.norm3(bev_tgt)
        return bev_tgt
...
class Interfuser(nn.Module):
    def __init__(
        ...
        self.salidency_model = model.get_model()().to(device=device)
        ...
        print('Using cross atten interfuser')
        ...
        self.bev_embed = nn.Parameter(torch.zeros(1, 1, embed_dim))
        ...
        self.fusion_layer = FusionLayer(embed_dim, num_heads, dim_feedforward, dropout, act_layer, normalize_before
        )
    ...
    def reset_parameters(self):
        ...
        nn.init.uniform_(self.bev_embed)
    ...
    def forward_features(
        ...
        # print(front_image_token.shape, self.view_embed[:, :, 0:1, :].shape)
        ...
        features_img.extend([front_image_token, front_image_token_global])
        ...
        if self.with_center_sensor:
            ...
            features_img.extend([front_center_image_token, front_center_image_token_global])
        ...
        if self.with_lidar:
            ...
            features_lidar.extend([lidar_token, lidar_token_global])

        features = torch.cat(features, 0)
        # features_img = torch.cat(features_img, 0)
        # features_lidar = torch.cat(features_lidar, 0)
        
        # bev_query = self.bev_embed.repeat(features.shape[0], features.shape[1], 1)

        # features_fusioned = self.fusion_layer(features, features_lidar, features_img)
        # import ipdb; ipdb.set_trace()
    ...
    def forward(self, x):
        ...
        # front_center_image = x["rgb_center"] # TODO replace it as salidency

        model_kwargs = {}
        this_frame_seq = x["rgb"].unsqueeze(0)
        raw_data = x["raw_data"]

        # 使用 interpolate 进行下采样
        raw_data = torch.nn.functional.interpolate(
            raw_data.permute(0, 3, 1, 2), size=(front_image.shape[2], front_image.shape[3])
        ).permute(0, 2, 3, 1)
        
        # model_kwargs['target_size'] = (x["rgb_center"].shape[1], x["rgb_center"].shape[2])
        model_kwargs['target_size'] = (raw_data.shape[1], raw_data.shape[2])
        ...
        smap = smap.exp()
        smap = smap.squeeze(0)
        smap = smap.expand(-1, 3, -1, -1).contiguous().permute(0, 2, 3 ,1)
        smap = utils.to_numpy(smap)
        smap = (smap / np.amax(smap) * 255).astype(np.uint8)

        # smap_color = cv2.applyColorMap(smap, cv2.COLORMAP_JET)
        # img_smap = Image.fromarray(cv2.cvtColor(smap, cv2.COLOR_BGR2RGB))
        # img_smap = Image.fromarray(smap[0])
        # raw_data = Image.fromarray(utils.to_numpy(raw_data)[0])
        # images = Image.blend(raw_data, img_smap, 0.6)
        # images.save('smap_save.png')
        
        cropped_image = torch.from_numpy(smap).permute(0, 3, 1, 2)

        # from (bs,h,w,c) -> (bs,c,h,w)
        salidency_transform = salidency_trans()
        salidency_transed_cropped_image = [salidency_transform(image) for image in cropped_image.byte()]
        salidency_transed_cropped_image = torch.stack(salidency_transed_cropped_image).to(device=device)

        front_center_image = conv_1(salidency_transed_cropped_image) + conv_1(x["rgb_center"])
        # print('time uses: %s ' % str(time.time() - starttime))
        # import ipdb;ipdb.set_trace()
